...
M.init = function(self, pm)
	...
	use({
		...
		config = function()
			...
			-- NOTE: [April 02, 2025] Disabled due to deeply shitty code that causes performance drop
			-- DO NOT ENABLE, UNTIL ISSUE RESOLVED AT:
			...
			config.options.offsets = std.Array:new()
			...
			if nvimtreeOk then
				...
				config.options.offsets:push(offset)
				config.options.custom_filter = function(buf_number, buf_numbers)
					if vim.bo[buf_number].filetype ~= "Nvim-tree" then
						return false
					end
