...
M.init = function(self, pm)
	...
	use({
		...
		config = function()
			...
			local package_infoOk, package_info = pcall(require, "package-info")
			if package_infoOk and package_info.get_status then
				table.insert(config.sections.lualine_c, package_info.get_status)
			end

			-- lsp-status.nvim integration
			local lsp_statusOk, lsp_status = pcall(require, "lsp-status")
			if lsp_statusOk and lsp_status.status then
				table.insert(config.sections.lualine_x, 1, lsp_status.status)
			end

			local profiler_ok, profiler = pcall(require, "hinell.profiler")
			if profiler_ok and lsp_statusOk and lsp_status.status then
