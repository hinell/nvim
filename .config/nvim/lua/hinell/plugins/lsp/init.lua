				...
				vim.lsp.buf.format({
					filter = function(client)
						...
						if client.name == "efm" then
							vim.notify("lsp formatting: efm server is used", vim.log.levels.INFO)
							return true
						end
						return true
					end
	...
	self.legendary.functions = {
		{
			...
			funcs = {
				...
				{
					description = "Editor: lsp: symbol rename",
					function()
						if #vim.lsp.get_clients() > 0 then
							vim.lsp.buf.rename()
						end
		...
		{ mode = { "n" }, description = "Workspace: lsp: find to references", vim.lsp.buf.references, opts = opts },
...
M.init = function(self, pm)
	...
	local on_attach = function(client, bufnr)
		...
		local lsp_statusOk, lsp_status = pcall(require, "lsp-status")
		if lsp_statusOk then
	...
	use({
		...
		config = function()
			...
			local lls_path = "/home/" .. os.getenv("USER") .. "/.local/share/nvim/lazy/lua-language-server/bin/lua-language-server"

			if uv.fs_stat(lls_path)
			then
	...
	use({
		...
		init = function(opts)
			...
			vim.g.lspTimeoutConfig = {
				-- stpTimeout  = 1000 * 4, -- ms, timeout before stopping all LSP servers
	...
	require("hinell.plugins.lsp.diagnostics"):init(pm)
