...
, { { "i" }, "<C-S-U>"   ,"<CMD>normal C<CR><Right>" , { desc="Line: delete until the end" } } -- CTRL+SHIFT+ALT+DOWN

---------------------------------------------------------------------navigation
--- Scrolling
, { { "n"      }, "<S-ENTER>", [[<CMD>echo "hinell: keymap:isabled"<CR>]] , { desc="Editor: scroll page (disabled)" } } -- SHIFT+ENTER
, { { "n", "v" }, "<S-Up>"   , [[<CMD>echo "hinell: keymap:isabled"<CR>]] , { desc="Editor: scroll page (disabled)" } } -- SHIFT+ENTER
, { { "n", "v" }, "<S-Down>" , [[<CMD>echo "hinell: keymap:isabled"<CR>]] , { desc="Editor: scroll page (disabled)" } } -- SHIFT+ENTER
--
--Scrolling
, { { "n", "v" }, "<C-U>", [[<CMD>echo "hinell: keymap:isabled"<CR>]] ,{ desc="Editor: scroll up   (disabled)" } } -- Disabled
, { { "n", "v" }, "<C-D>", [[<CMD>echo "hinell: keymap:isabled"<CR>]] ,{ desc="Editor: scroll down (disabled)" } } -- Disabled
		...
		keymaps = {
			...
			{ { "n" }, "<C-g>cia", function()
				...
				vim.ui.input({ prompt = "git commit: skip message editing? (y/N)): "}, function(input)
					...
					print(vim.inspect(command_list))
					local command_stdx_cb = function(output) print(output) end
			...
			end, { desc="file: git unstage / reset" } }, -- CTRL-g un

			{ { "n" }, "<C-g>fco" , function()
				if vim.bo.buftype ~= "" then return end
				local file = M.buf_get_file_name()
				if file then
					vim.ui.input({ prompt = file .. " - checkout? (y/N)): "}, function(input)
			...
			end, { desc="file: git checkout to HEAD" } } -- CTRL-g un
...
M.keymaps = vim.list_extend(M.keymaps, {
	...
	, { { "n", "v", "i", "x" , "o" }, "<F28>", function()
		-- vim.cmd("<CMD>bdelete!<CR>")
		local tabCurrent = vim.api.nvim_get_current_tabpage()
		local tabWindows = vim.api.nvim_tabpage_list_wins(tabCurrent)
		for index, winNr in ipairs(tabWindows) do
			local winIsValid, buf = pcall(vim.api.nvim_win_get_buf, winNr)
			if winIsValid then
				vim.api.nvim_buf_delete(buf, { force = false, unload = false })
			end
		end
		if tabCurrent > 1 then
			vim.cmd("tabprevious")
		end
		vim.notify("windows closed: " .. #tabWindows, vim.log.levels.INFO)
